...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
     ...
     include '../../config/db_config.php';
        ...
        VALUES ('$sale4', '$product4', '$SEshopno', '$p4price', '$ammount4', '$unit4', '$p4total', '$today', '$SEshop_id')";
		if ($conn->query($sql) === TRUE) {
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
     ...
     include '../../config/db_config.php';
	    $sql = "INSERT INTO sales(sales_id, product_name, shop, price, amount, unit, total, date, signature)
        VALUES ('$sale5', '$product5', '$SEshopno', '$p5price', '$ammount5', '$unit5', '$p5total', '$today', '$SEshop_id')";
		if ($conn->query($sql) === TRUE) {
